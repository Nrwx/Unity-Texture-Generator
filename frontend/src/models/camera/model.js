...
const normalizeCamera = camera => ({
    ...
    orthographic_scale: toNumber(
        camera?.orthographic_scale ?? camera?.orthographicScale,
        DEFAULT_CAMERA.orthographic_scale
    ),
    orthographicScale: toNumber(
        camera?.orthographicScale ?? camera?.orthographic_scale,
        DEFAULT_CAMERA.orthographicScale
    ),
});
...
export function cameraModel(props, emit) {
    ...
    const activeLayer = computed(() => (
        ...
        (props.layers || []).find(layer => (
            layer.id === animatorActiveLayerId.value &&
            Number(layer?.type) === 5
        )) ||
        null
    ));
    ...
    const setSavedCameraField = (key, value) => {
        ...
        camera[key] = key === "projection"
            ? value
            : toNumber(value, DEFAULT_CAMERA[key] ?? 0);

        if (key === "orthographicScale") {
            camera.orthographic_scale = camera.orthographicScale;
        }

        if (key === "orthographic_scale") {
            camera.orthographicScale = camera.orthographic_scale;
        }

        if (key === "projection") {
            animatorCameraCommand.projection = camera.projection;
        }
