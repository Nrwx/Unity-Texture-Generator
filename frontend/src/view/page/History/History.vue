  ...
  <v-card :theme="theme" class="pa-0" width="100%" flat height="100%" v-for="(tab, index) in tabs" :key="tab.name" v-show="tabIndex === index">
    ...
    <template v-if="tabIndex === 0">
      <v-card-title class="headline d-flex align-center">
        Zeitbasierte Protokollsteuerung
      </v-card-title>
      <v-card-subtitle>
      ...
      <v-card-text class="map-grid overflow-hidden overflow-y-auto" style="height: 80vh; max-height: 80vh; max-width: 100%;">
        <v-list dense>
          <v-list-item
    ...
    <template v-if="tabIndex === 1">
      ...
      <v-card-text style="padding: 0; max-width: 100%;">
        ...
        <div v-if="sortedBuilds.length" class="map-grid overflow-hidden overflow-y-auto" style="height: 80vh; max-height: 80vh; max-width: 100%;">
          <v-col
...
export default defineComponent({
  ...
  props: historyProps,
  setup(props, { emit }) {
    const { theme, emitEvent, sortedBuilds, sortOptions, toggleCollapse, tabIndex, handleTabEmit, tabs } = historyModel(props, emit);
    return {
      theme,
      localData,
      backupStates,
