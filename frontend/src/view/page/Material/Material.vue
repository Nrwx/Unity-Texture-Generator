      ...
      <div class="mem-content" :class="{ 'workspace-full': isFullWorkspaceTab }">
        ...
        <section class="mem-workspace">
          ...
          <div class="mem-view relative">
            ...
            <!-- SHADER -->
            <Shader
                v-else-if="ui.activeTab === 'shader'"
                v-model:shader-graph="values.shader_graph"
                v-model:surface="values.surface"
                v-model:bitmap-maps="values.bitmap_maps"
                v-model:uv="values.uv"
                :texture-layers="textureLayers"
                :selected-source-layer="selectedSourceLayer"
                :layer="layer"
                @request-preview="requestPreviewDebounced"
                @change="requestPreviewDebounced"
                @update:surface-offsets="syncSurfaceOffsetsFromNodes"
                @update:surface-slot-from-uv-cubemap="({ slotKey }) => syncSurfaceSlotFromUvCubeMap(slotKey)"
                @update:texture-settings-to-uv-faces="({ slotKey, settings }) => syncTextureSettingsToUvFaces(slotKey, settings)"
                @update:texture-group-settings-to-uv-faces="({ slotKey, group, settings }) => syncTextureGroupSettingsToUvFaces(slotKey, group, settings)"
            />
...
import Shader from "@/view/page/Material/Shader/Shader";
